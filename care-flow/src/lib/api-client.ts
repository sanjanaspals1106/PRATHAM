    ...
    this.client.interceptors.response.use(
      (response) => {
        ...
        if (response.data && response.data.priority) {
          response.data.priority = toFrontendPriority(response.data.priority);
        }

        // Convert priorities in array responses
        if (response.data && response.data.recentEntries) {
          response.data.recentEntries = response.data.recentEntries.map((entry: any) => ({
            ...entry,
